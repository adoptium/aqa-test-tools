...
export default class TabularView extends Component {
    ...
    async initializeJdk() {
        ...
        */
        this.generateDropdown(
            'testJdkVersion',
            this.state.tabularDropdown['jdkVersion'],
            this.state.defaultValues.jdkVersion
        );
        this.generateDropdown(
            'testJvmType',
            this.state.tabularDropdown['jvmType'],
            this.state.defaultValues.jvmType
        );
        this.generateDropdown(
            'testSdkResource',
            this.state.tabularDropdown['sdkResource'],
            this.state.defaultValues.testSdkResource
        );
        this.generateDropdown(
            'testBuildServer',
            this.state.tabularDropdown['buildServer'],
            this.state.defaultValues.jenkinsServer
        );
        this.generateDropdown(
            'baselineJdkVersion',
            this.state.tabularDropdown['jdkVersion'],
        ...
        this.generateDropdown(
            'baselineSdkResource',
            this.state.tabularDropdown['sdkResource'],
            this.state.defaultValues.baselineSdkResource
        );
        this.generateDropdown(
            'baselineBuildServer',
            this.state.tabularDropdown['buildServer'],
    ...
    async handleSubmit(event) {
        ...
        const newPath =
            ...
            this.state.testJdkDate +
            '&testJvmType=' +
            this.state.testJvmType +
            '&testJdkVersion=' +
            this.state.testJdkVersion +
            '&testSdkResource=' +
            this.state.testSdkResource +
            '&testBuildServer=' +
            this.state.testBuildServer +
            '&baselineJdkDate=' +
            this.state.baselineJdkDate +
            '&baselineJvmType=' +
            this.state.baselineJvmType +
            '&baselineJdkVersion=' +
            this.state.baselineJdkVersion +
            '&baselineSdkResource=' +
            this.state.baselineSdkResource +
            '&baselineBuildServer=' +
            this.state.baselineBuildServer;
    ...
    onPlatformChange(checkedValues, event) {
        ...
        newArray = newArray.filter(
            (column) =>
                column.Header === 'Benchmark Name' ||
                checkedValues.includes(column.id)
        );
    ...
    generateColumns(platforms) {
        ...
        let column = {
            ...
            Cell: (props) => (
                <span>
                    {props.value.split(',')[0]} <br />{' '}
                    {props.value.split(',')[1]} <br />{' '}
                    {props.value.split(',')[2]}
                </span>
            ),
        };
        ...
        for (let i = 0; i < platforms.length; i++) {
            ...
            column.Cell = (props) => (
                <Tooltip
                    title={
                        <div>
                            Test Raw Score:{' '}
                            {this.handleProp(props.value, 'testScore')} <br />
                            Test CI: {this.handleProp(
                                props.value,
                                'testCI'
                            )}{' '}
                            <br />
                            Test JDK Date:{' '}
                            {this.handleProp(props.value, 'testJdkDate')} <br />
                            Test Sdk Resource:{' '}
                            {this.handleProp(
                                props.value,
                                'testSdkResource'
                            ...
                            <br />
                            Baseline Raw Score:{' '}
                            {this.handleProp(props.value, 'baselineScore')}{' '}
                            <br />
                            Baseline CI:{' '}
                            {this.handleProp(props.value, 'baselineCI')} <br />
                            Baseline JDK Date:{' '}
                            {this.handleProp(
                                props.value,
                                'baselineJdkDate'
                ...
                >
                    <span
                        onClick={() =>
                            this.handleLink(
                                this.handleProp(props.value, 'buildUrl')
                            )
                        }
                    >
                        {' '}
                        ...
                        {this.handleCI(
                            this.handleProp(props.value, 'totalCI'),
                            this.handleProp(props.value, 'relativeComparison')
                        )}{' '}
                    </span>
                </Tooltip>
            );
            column.getProps = (state, rowInfo) =>
                this.handleRegression(
                    this.handleProp(rowInfo.row[platform], 'relativeComparison')
                );
            newArray.push(column);
        }
    ...
    colorFilter(firstFilter) {
        ...
        if (firstFilter) {
            ...
            let newArray = JSON.parse(
                JSON.stringify(this.state.consolidatedData)
            );
            let filterRange;
            // If color filter is set to ALL, do not not apply filter
            if (this.state.colorFilter === 'green') {
                filterRange = greenFilter;
            } else if (this.state.colorFilter === 'yellow') {
                filterRange = yellowFilter;
            } else if (this.state.colorFilter === 'red') {
            ...
            for (let i = 0; i < this.state.consolidatedData.length; i++) {
                for (let platform in this.state.consolidatedData[i]
                    .platformsSpecificData) {
                    // Set values to N/A if they are outside the filterRange
                    if (
                        parseFloat(
                            this.state.consolidatedData[i]
                                .platformsSpecificData[platform]
                                .relativeComparison
                        ) > filterRange[1] ||
                        parseFloat(
                            this.state.consolidatedData[i]
                        ...
                        newArray[i].platformsSpecificData[
                            platform
                        ].relativeComparison = 'N/A';
                    }
                }
    ...
    populateTable(data, type) {
        ...
        data.forEach(
            function (testResultsObject) {
                const buildInfo = getInfoFromBuildName(
                    testResultsObject.buildName
                );
                if (buildInfo) {
                    ...
                    for (
                        ...
                        for (const metric in testResultsObject.aggregateInfo[
                            aggregateIndex
                        ].metrics) {
                            found = false;
                            benchmarkNVM =
                            ...
                            for (const currentDataObject in newArray) {
                                ...
                                if (
                                    newArray[currentDataObject].benchmarkNVM ===
                                    benchmarkNVM
                                ) {
                                    found = true;
                                    newArray[
                            ...
                            if (!found) {
                                ...
                                dataObject.platformsSpecificData[platform] =
                                    this.handleEntry(
                                        aggregateIndex,
                                        testResultsObject,
                                        metric,
    ...
    populateCompTable = async () => {
        ...
        for (let testDataObject of this.state.testData) {
            ...
            let matchingDataObject = this.state.baselineData.find(
            ...
            if (matchingDataObject != null) {
                Object.keys(testDataObject.platformsSpecificData).forEach(
                    function (platform) {
                        // Baseline data contains same benchmark and same platform, compare values and store in comparison table
                        if (
                            matchingDataObject.platformsSpecificData.hasOwnProperty(
                                platform
                            )
                        ) {
                            consolidatedDataObject.platformsSpecificData[
        ...
        this.state.baselineData.forEach(function (baselineDataObject) {
            ...
            if (matchingDataObject != null) {
                Object.keys(baselineDataObject.platformsSpecificData).forEach(
                    function (platform) {
                        if (
                            !matchingDataObject.platformsSpecificData.hasOwnProperty(
                                platform
    ...
    showData = async (type) => {
        ...
        if (type === 'test') {
            info =
                await fetchData(`/api/getTabularData?jdkVersion=${this.state.testJdkVersion}&jvmType=${this.state.testJvmType}&jdkDate=${this.state.testJdkDate}
            &sdkResource=${this.state.testSdkResource}&buildServer=${this.state.testBuildServer}`);
        } else {
            info =
                await fetchData(`/api/getTabularData?jdkVersion=${this.state.baselineJdkVersion}&jvmType=${this.state.baselineJvmType}&jdkDate=${this.state.baselineJdkDate}
        ...
        for (let platform in this.state.platforms) {
            platformFilter.push({
                label: this.state.platforms[platform],
                value: this.state.platforms[platform],
                disabled: false,
            });
        }
            ...
            <div>
                ...
                <div className="row">
                    ...
                    <div className="column">
                        ...
                        <select
                            ...
                            value={this.state.testJdkVersion}
                    ...
                    <div className="column">
                        ...
                        <select
                            ...
                            className="select-css"
                            value={this.state.baselineJdkVersion}
                            onChange={this.handleChange.bind(this)}
                        ></select>
                    </div>
                </div>
                <div className="row">
                    ...
                    <div className="column">
                        ...
                        <select
                            ...
                            className="select-css"
                            value={this.state.baselineJvmType}
                            onChange={this.handleChange.bind(this)}
                        ></select>
                    </div>
                </div>
                <div className="row">
                    ...
                    <div className="column">
                        ...
                        <select
                            ...
                            className="select-css"
                            value={this.state.testSdkResource}
                            onChange={this.handleChange.bind(this)}
                        ></select>
                    </div>
                    <div className="column">
                        ...
                        <select
                            ...
                            className="select-css"
                            value={this.state.baselineSdkResource}
                            onChange={this.handleChange.bind(this)}
                        ></select>
                    </div>
                    <div className="row">
                        ...
                        <div className="column">
                            ...
                            <select
                                ...
                                className="select-css"
                                value={this.state.testBuildServer}
                                onChange={this.handleChange.bind(this)}
                            ></select>
                        </div>
                        <div className="column">
                            ...
                            <select
                                ...
                                className="select-css"
                                value={this.state.baselineBuildServer}
                                onChange={this.handleChange.bind(this)}
                            ></select>
                        </div>
                    </div>
                ...
                <Collapse>
                    <Panel header="Filters" key="1">
                        ...
                        <Checkbox.Group
                            options={this.state.platformFilter}
                        ...
                        <select
                            ...
                            onChange={this.handleColorFilter.bind(this)}
                        >
