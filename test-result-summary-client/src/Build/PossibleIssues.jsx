    ...
    async fetchIssues() {
        ...
        if (response.ok) {
            ...
            for (let index = 0; index < relatedIssues.items.length; index++) {
                ...
                const issue_creator = <a href={relatedIssues.items[index].user.html_url} target="_blank" rel="noopener noreferrer">{relatedIssues.items[index].user.login}</a>;
