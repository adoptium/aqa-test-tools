  ...
  async fetchIssues() {
    ...
    if (response.ok) {
      ...
      for (let index = 0; index < relatedIssues.items.length; index++) {
        const repoName = relatedIssues.items[index].repository_url.replace(
          repoUrlAPIPrefix,
          ""
        );
        ...
        const userFeedback = (
          <>
            ...
            >
              <SmileOutlined style={{ fontSize: "25px", color: "green" }} />
            </Button>
            &nbsp;
            <Button
              onClick={this.getUserFeedback(
            ...
            >
              <FrownOutlined style={{ fontSize: "25px", color: "red" }} />
            </Button>
          </>
        );
