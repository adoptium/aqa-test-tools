...
export default class SearchResult extends Component {
    ...
    async updateData() {
        ...
        const buildData = builds.map(( element ) => {
            const ret = {
                ...
                jenkinsBuild: { buildName: element.buildName, buildNum: element.buildNum, buildUrl: element.buildUrl, url: element.url },
                result: { _id: element._id, buildResult: element.buildResult },
                resultDetail: element.testSummary,
                date: element.timestamp ? new Date( element.timestamp ).toLocaleString() : null
            }
            return ret;
        } );

        buildData.sort(( a, b ) => {
            let rt = a.buildData.buildResult.localeCompare( b.buildData.buildResult );
            if ( rt === 0 ) {
    ...
    render() {
        ...
        return <BuildTable title={'Found ' + buildData.length + ' "' + searchText + '" in build output'} buildData={buildData} />
    }
}