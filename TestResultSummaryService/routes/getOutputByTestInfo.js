...
module.exports = async ( req, res ) => {
    const { url, buildName, buildNum, testName, removeTimestampFlag, applyDeepSmithMatchFlag } = req.query;
    ...
    if ( data[0] ) {
        let result = await outputDB.getData( { _id: new ObjectID( data[0].tests.testOutputId ) } ).toArray();
        try {
