...
module.exports = async (req, res) => {
    ...
    const build = await testResultsDB.getSpecificData(query, {
        _id: 1,
        buildName: 1,
        buildNum: 1,
        buildResult: 1,
    ...
    const testOutputIds = await outputDB.aggregate([
        {
            $match: {
                ...
                _id: {
                    $in: Object.keys(allOutputIds.testOutputIds).map(
    ...
    const buildOutputIds = await outputDB.aggregate([
        {
            $match: {
                output: { $regex: searchText, $options: 'i' },
                _id: {
                    $in: Object.keys(allOutputIds.buildOutputIds).map(
                        (entry) => new ObjectID(entry)
                    ),
                },
            },
    ...
    const tests = testOutputIds.map(
        (key) => allOutputIds.testOutputIds[key._id]
    );

    const builds = buildOutputIds.map(
        (key) => allOutputIds.buildOutputIds[key._id]
    );
...
searchOutputId = async (build) => {
    ...
    if (build.buildOutputId) {
        rt.buildOutputIds[build.buildOutputId] = {
            _id: build._id,
    ...
    if (!build.hasChildren) {
        if (build.tests) {
            build.tests.forEach((test) => {
                rt.testOutputIds[test.testOutputId] = {
                    _id: test._id,
                    buildName: build.buildName,
                    buildNum: build.buildNum,
                    buildUrl: build.buildUrl,
        ...
        let builds = await testResultsDB.getSpecificData(
            { parentId: new ObjectID(build._id) },
            {
                _id: 1,
                buildName: 1,
                buildNum: 1,
        ...
        await Promise.all(
            builds.map(async (b) => {
                ...
                if (res.testOutputIds) {
                    rt.testOutputIds = Object.assign(
                        {},
                        rt.testOutputIds,
                        res.testOutputIds
                    );
                ...
                if (res.buildOutputIds) {
                    rt.buildOutputIds = Object.assign(
                        {},
                        rt.buildOutputIds,
                        res.buildOutputIds
                    );
