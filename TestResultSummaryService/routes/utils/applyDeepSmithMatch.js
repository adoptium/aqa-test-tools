function applyDeepSmithMatch(inputText) {
    ...
    inputText = inputText.substring(
        inputText.indexOf(startWords),
        inputText.lastIndexOf(endWords)
    );
    if (!inputText) {
    ...
    for (let index = 0; index < testsAll.length; index++) {
        ...
        if (testParts.length !== testPartsTotal) {
            ...
            if (exceptionName === null) {
                ...
                let printExceptionName = exceptionName[0].substring(
                    0,
                    exceptionName[0].length - 1
                );
                curOutput +=
                    testKey + 'EXCEPTION is: ' + printExceptionName + '\n';
            }
            // Part 3 output
            if (ignoreResultFlag) {
                curOutput +=
                    testKey + 'OUTPUT is ' + ignoreResultWord + '_value \n\n';
            } else {
